...
export function RecordsTable() {
  ...
  // Load user list for admin filter dropdown
  useEffect(() => {
    if (!isAdmin) return
  ...
  const loadRecords = useCallback(async () => {
    ...
    try {
      ...
      // Non-admin: only own records (belt-and-suspenders with RLS)
      if (!isAdmin) query = query.eq('user_id', user.id)
      if (isAdmin && userFilter) query = query.eq('user_id', userFilter)
      ...
      const { data: bills, count, error: billErr } = await query

      if (billErr) {
        console.error('bill_records fetch error:', billErr)
        throw new Error(billErr.message || billErr.code || 'Failed to load records')
      }
      ...
      const enriched = (bills ?? []).map((b: any) => ({
        ...b,
        _name: nameMap[b.user_id] || '—',
      }))

      setRows(enriched)
      setTotal(count ?? 0)
    } catch (e: any) {
      const msg = e?.message || 'Unknown error loading records'
      console.error('loadRecords error:', msg)
      setFetchErr(msg)
    } finally {
      setBusy(false)
    }
  }, [user?.id, isAdmin, page, search, dateFrom, dateTo, userFilter])

  useEffect(() => { loadRecords() }, [loadRecords])

  // ── Handlers ───────────────────────────────────────────────────
  const deleteRow = async (id: string, contact: string) => {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button variant="outline" size="sm" onClick={exportUnique}>
            <Download className="h-4 w-4" /> Unique
          </Button>
          <Button variant="outline" size="sm" onClick={exportAll}>
            <Download className="h-4 w-4" /> All
          </Button>
          <Button variant="outline" size="sm" onClick={loadRecords} loading={busy}>
      ...
      <div className="relative">
        ...
        <input
          className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white"
      ...
      {fetchErr && (
        <div className="flex items-center gap-2 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl text-sm text-red-700 dark:text-red-400">
          ...
          <button onClick={loadRecords} className="underline text-xs">Retry</button>
        </div>
      )}
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-16 text-center">
                    ...
                    <p className="text-gray-400 text-sm">
                      {fetchErr ? 'Error loading records' : 'No records found'}
                    </p>
                    {!search && !dateFrom && !fetchErr && (
                      <p className="text-gray-300 dark:text-gray-600 text-xs mt-1">
                        Capture some bills to see them here
                      </p>
                    )}
                  </td>
                </tr>
                ...
                return (
                  <React.Fragment key={r.id}>
                    <tr className="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/20">
                      ...
                      <td className="px-3 py-3">
                        {editing ? (
                          <input value={editForm.customer_name || ''} onChange={e => setEditForm((f: any) => ({ ...f, customer_name: e.target.value }))}
                            className="w-full px-2 py-1 rounded border border-blue-400 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
                        ) : (
                      ...
                      <td className="px-3 py-3">
                        {editing ? (
                          <input value={editForm.contact_number || ''} onChange={e => setEditForm((f: any) => ({ ...f, contact_number: e.target.value }))}
                            className="w-full px-2 py-1 rounded border border-blue-400 text-sm font-mono bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
                        ) : (
                      ...
                      <td className="px-3 py-3 hidden md:table-cell">
                        {editing ? (
                          <input value={editForm.bill_number || ''} onChange={e => setEditForm((f: any) => ({ ...f, bill_number: e.target.value }))}
                            className="w-full px-2 py-1 rounded border border-blue-400 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white" />
                        ) : (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {editing ? (
                            ...
                            <>
                              ...
                              <button
                                onClick={() => { setEditId(r.id); setEditForm({ customer_name: r.customer_name || '', contact_number: r.contact_number || '', bill_number: r.bill_number || '' }) }}
                                className="p-1.5 rounded hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-500"><Edit2 className="h-4 w-4" /></button>
                              <button onClick={() => deleteRow(r.id, r.contact_number)}
                                className="p-1.5 rounded hover:bg-red-50 dark:hover:bg-red-900/20 text-red-500"><Trash2 className="h-4 w-4" /></button>
                            </>
                          )}
                        </div>
                      </td>
                      ...
                      <tr className="border-b border-gray-100 dark:border-gray-700/50 bg-blue-50/20 dark:bg-blue-900/5">
                        <td colSpan={6} className="px-4 py-3">
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                            <div>
                              <span className="text-gray-400 block mb-0.5">Bill #</span>
                              <span className="font-medium text-gray-900 dark:text-white">{r.bill_number || '—'}</span>
                            </div>
                            <div>
                              <span className="text-gray-400 block mb-0.5">Captured By</span>
                              <span className="font-medium text-gray-900 dark:text-white">{r._name}</span>
                            </div>
                            <div>
                              <span className="text-gray-400 block mb-0.5">Captured At</span>
                              <span className="font-medium text-gray-900 dark:text-white">{formatDateTime(r.created_at)}</span>
                            </div>
                            <div>
                            ...
                            <div>
                              <span className="text-gray-400 block mb-0.5">OCR Mode</span>
                              <Badge variant={r.ocr_mode === 'ai' ? 'info' : 'default'}>{r.ocr_mode || '—'}</Badge>
                            </div>
                            <div>
                              <span className="text-gray-400 block mb-0.5">Confidence</span>
                              <span className="font-medium text-gray-900 dark:text-white">
                                {r.ocr_confidence ? `${Number(r.ocr_confidence).toFixed(0)}%` : '—'}
                              </span>
                            </div>
                          </div>
                        </td>
